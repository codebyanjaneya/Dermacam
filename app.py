...
def analyze():
    ...
    if brightness < 40:
        print(f"⚠️  Image too dark: {brightness:.1f}")
        return jsonify({
            **ERROR_FALLBACK,
            "diagnosis":    "Image Too Dark",
            "what_is_this": "📸 Photo bahut dark hai. Please natural light ya bright indoor light mein photo lo. Andheron mein scan accurate nahi hoga.",
            "confidence":   0,
            "error":        "low_brightness"
        })

    if brightness > 240:
        print(f"⚠️  Image overexposed: {brightness:.1f}")
        return jsonify({
            **ERROR_FALLBACK,
            "diagnosis":    "Image Too Bright",
            "what_is_this": "📸 Photo overexposed hai. Flash band karo aur thoda door se photo lo.",
            "confidence":   0,
            "error":        "overexposed"
        })

    print(f"✅ Image quality OK — brightness: {brightness:.1f}")
